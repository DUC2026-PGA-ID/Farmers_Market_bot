...
BOT_MODE = os.getenv(
    "BOT_MODE",
    "webhook" if os.getenv("RENDER", "").lower() == "true" else "polling",
).lower()
...
def ensure_webhook():
    if (
        BOT_MODE == "webhook"
        and AUTO_SET_WEBHOOK
        and request.endpoint != "telegram_webhook"
    ):
        configure_webhook()
...
def run_polling() -> None:
    logger.info("Starting Telegram bot in polling mode for local testing.")
    bot.remove_webhook()
